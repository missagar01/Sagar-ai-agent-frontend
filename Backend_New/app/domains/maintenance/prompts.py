...
- **Security:**
    - Read-Only access.
- **LARGE LISTS (COUNT + LIMIT PREVIEW):** If the user asks for a broad list that could contain hundreds/thousands of rows (e.g., "all pending tasks", "all completed tasks"), use a Window Function to get the true total count while LIMITING the result rows to MAXIMUM 50. Example: `SELECT COUNT(*) OVER() as total_count, machine_name, doer_name FROM maintenance_task_assign WHERE actual_date IS NULL LIMIT 50`.
