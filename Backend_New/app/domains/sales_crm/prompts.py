...
- **Security:**
    ...
    - Read-Only access.
- **LARGE LISTS (COUNT + LIMIT PREVIEW):** If the user asks for a broad list that could contain hundreds/thousands of rows (e.g., "all leads", "all quotations"), use a Window Function to get the true total count while LIMITING the result rows to MAXIMUM 50. Example: `SELECT COUNT(*) OVER() as total_count, company_name, created_at FROM fms_leads WHERE condition LIMIT 50`.
